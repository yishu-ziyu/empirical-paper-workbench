...
| FD-fetch-card-not-fixture | Card zip fetch must not treat classic-5 / proximity.zip as the NJ–PA archive | Same author page lists schooling proximity.zip; fixture CSV can be served as a fake download | Only `njmin.zip`; zip magic required; else landing + honest upload; no `dataAttached` | `agent-learning/raw/2026-09-15_fd-be-fetch-card.md` |
| FD-dataverse-fetch-not-fixture | Dataverse miss/restricted/HTML must not become a fixture find | Fetch-dataverse owns live catalog bytes or a followable URL | Search miss → `external_link` Dataverse path; restricted/HTML → `discovered` + `link_only`; public file → session staging, not `dataAttached` | `agent-learning/raw/2026-09-15_fd-be-fetch-dataverse.md` |
...
| REAL-FETCH-fold-on-bryce | Honesty + Card/Dataverse/WDI fetch lived on G0 siblings, not live BRYCE tip | Product write-sets disjoint; STATE/wiki/OpenAPI overlap | Serial-merge onto BRYCE; keep data_honesty/attach/ck1994; fixtures teaching-shelf only; regen OpenAPI | `agent-learning/raw/2026-09-15_fm-e-fold-real-fetch-1.md` |
