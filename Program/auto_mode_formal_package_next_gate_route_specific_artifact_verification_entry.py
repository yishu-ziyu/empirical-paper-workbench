...
from Program.workbench.auto_mode_formal_package_next_gate_route_specific_artifact_verification_entry import (  # noqa: E402
    DEFAULT_ENTRY_PATH,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Enter existing route-specific artifact verification after P7-AS result review."
    )
    parser.add_argument("--project-root", default=".")
    parser.add_argument("--route-specific-artifact-execution-result-review", default=str(DEFAULT_RESULT_REVIEW_PATH))
    parser.add_argument("--output-entry", default=str(DEFAULT_ENTRY_PATH))
    parser.add_argument("--output-review", default=str(DEFAULT_REVIEW_PATH))
    args = parser.parse_args()

    project_root = Path(args.project_root).resolve()
    result_review_path = Path(args.route_specific_artifact_execution_result_review)
    result_review = load_json_or_empty(project_root / result_review_path)
    report, _exit_code = run_auto_mode_formal_package_next_gate_route_specific_artifact_verification_entry(
        project_root,
        result_review,
        source_paths={
            "route_specific_artifact_execution_result_review": str(result_review_path),
        },
        repo_root=REPO_ROOT,
    )
    ...
    print(
        "[econ-workbench] "
        "auto_mode_formal_package_next_gate_route_specific_artifact_verification_entry="
        f"{report_path.relative_to(project_root)}"
    )
    print(
        "[econ-workbench] "
        "auto_mode_formal_package_next_gate_route_specific_artifact_verification_entry_review="
    ...
    print(
        "[econ-workbench] "
        f"verification_artifact_record_count={report['verification_artifact_record_count']}"
    )
