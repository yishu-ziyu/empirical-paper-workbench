...
SOURCE_READY_STATUS = "ready_for_manifested_routed_downstream_execute_result_continuation_result_review_continuation_gate_entry"
...
EXPORT_READY_TO_ENTER_STATUS = (
    "ready_to_enter_manifested_routed_downstream_execute_result_continuation_result_review_"
...
def run_auto_mode_formal_package_next_gate_manifested_routed_downstream_execute_result_continuation_result_review_continuation_gate_entry_execute_gate(
    project_root: Path,
    ...
    *,
    ...
    route_entry_report, route_entry_exit_code = (
        run_auto_mode_formal_package_next_gate_route_specific_artifact_execution(
            project_root,
            build_route_specific_artifact_execution_source(gate_entry, report["continuation_input_record"]),
            mode="dry-run",
            reviewer=reviewer,
            note=note,
            source_paths={
                "route_specific_artifact_executor_entry_result_review": (
                    "derived_from_p7_bp_downstream_execute_result_continuation_result_review_continuation_gate_entry"
                ),
            },
            repo_root=repo_root,
        )
    ...
    route_entry_report_path, route_entry_review_path = (
        write_auto_mode_formal_package_next_gate_route_specific_artifact_execution_outputs(
            project_root,
            route_entry_report,
            Path(report["route_specific_artifact_execution_report_path"]),
            Path(report["route_specific_artifact_execution_review_path"]),
        )
    )

    report["continuation_execute_command_executed"] = True
    report["this_command_ran_continuation_command"] = True
    report["route_specific_artifact_execution_returncode"] = route_entry_exit_code
    report["route_specific_artifact_execution_status"] = route_entry_report.get("status", "")
    ...
    if route_entry_exit_code == 0 and route_entry_report.get("status") == "route_specific_artifact_execution_dry_run_ready":
        report["status"] = EXPORT_ENTERED_STATUS
        report["blocking_reasons"] = []
        report["route_specific_artifact_execution_entered"] = True
    ...
    report["status"] = (
        ...
        "route_specific_artifact_execution_failure"
    )
    report["blocking_reasons"] = dedupe(
        report["blocking_reasons"]
        + [
            "route_specific_artifact_execution_failed",
            f"route_specific_artifact_execution_status:{route_entry_report.get('status', 'missing')}",
        ]
    )
...
def build_auto_mode_formal_package_next_gate_manifested_routed_downstream_execute_result_continuation_result_review_continuation_gate_entry_execute_gate(
    project_root: Path,
    gate_entry: dict[str, Any],
    *,
    mode: str = "dry-run",
    ...
    product_review_records = (
        [build_product_review_packet_continuation_record(record, reviewer, note)]
    ...
    return {
        ...
        "source_paths": {
            "manifested_routed_downstream_execute_result_continuation_result_review_continuation_gate_entry": source_paths.get(
        ...
        "can_execute_downstream_execute_result_continuation_result_review_continuation_with_confirmation": can_execute,
        ...
        "route_specific_artifact_execution_entered": False,
        "route_specific_artifact_execution_report_path": record.get("next_report_path", "") if can_execute else "",
        "route_specific_artifact_execution_review_path": record.get("next_review_path", "") if can_execute else "",
        "route_specific_artifact_execution_returncode": None,
        "route_specific_artifact_execution_status": "",
        "route_specific_artifact_execution_result": {},
        "product_review_packet_continuation_recorded": status == MANUAL_RECORDED_STATUS,
        "product_review_packet_continuation_records": product_review_records,
...
def build_source_blocking_reasons(gate_entry: dict[str, Any]) -> list[str]:
    ...
    if gate_entry.get("schema_version") != GATE_ENTRY_SCHEMA_VERSION:
        reasons.append("manifested_routed_downstream_execute_result_continuation_result_review_continuation_gate_entry_missing_or_invalid_schema")
    if gate_entry.get("status") != SOURCE_READY_STATUS:
        reasons.append("manifested_routed_downstream_execute_result_continuation_result_review_continuation_gate_entry_not_ready")
    if gate_entry.get("downstream_execute_result_continuation_result_review_gate_entry_recorded") is not True:
        reasons.append("manifested_routed_downstream_execute_result_continuation_result_review_continuation_gate_entry_not_recorded")
    if gate_entry.get("can_request_downstream_execute_result_continuation_result_review_continuation") is not True:
        reasons.append("manifested_routed_downstream_execute_result_continuation_result_review_continuation_gate_entry_cannot_request_continuation")
    ...
    if gate_entry.get("continuation_kind") not in {
        "route_specific_artifact_execution_continuation",
...
def build_boundary_blocking_reasons(gate_entry: dict[str, Any]) -> list[str]:
    ...
    if gate_entry.get("selected_route_executed") is True:
        reasons.append("downstream_execute_result_continuation_result_review_continuation_gate_entry_selected_route_executed")
    if gate_entry.get("export_or_acceptance_executed") is True:
        reasons.append(
            "downstream_execute_result_continuation_result_review_continuation_gate_entry_executed_export_or_acceptance"
        )
    if gate_entry.get("rendered_pdf") is True:
        reasons.append("downstream_execute_result_continuation_result_review_continuation_gate_entry_rendered_pdf")
    if gate_entry.get("rendered_docx") is True:
        reasons.append("downstream_execute_result_continuation_result_review_continuation_gate_entry_rendered_docx")
    if gate_entry.get("package_manifest_generated") is True:
        reasons.append(
            "downstream_execute_result_continuation_result_review_continuation_gate_entry_generated_package_manifest"
        )
    if gate_entry.get("manual_acceptance_performed") is True:
        reasons.append(
            "downstream_execute_result_continuation_result_review_continuation_gate_entry_performed_manual_acceptance"
        )
    if gate_entry.get("formal_writeback_executed") is True:
        reasons.append("downstream_execute_result_continuation_result_review_continuation_gate_entry_formal_writeback")
    if gate_entry.get("this_command_wrote_formal_state") is True:
        reasons.append("downstream_execute_result_continuation_result_review_continuation_gate_entry_wrote_formal_state")
...
def build_continuation_input_contract_blocking_reasons(gate_entry: dict[str, Any]) -> list[str]:
    ...
    record = records[0]
    if gate_entry.get("continuation_kind") == "route_specific_artifact_execution_continuation":
...
def build_export_continuation_record_blocking_reasons(
    ...
    if route_type not in EXPORT_ROUTE_TYPES:
        reasons.append(f"route_specific_artifact_execution_continuation_route_type_not_allowed:{route_type}")
    ...
    if record.get("continuation_kind") != "route_specific_artifact_execution_continuation":
        reasons.append(f"continuation_input_record_kind_mismatch:{route_type}")
    if record.get("review_status") != "route_specific_artifact_execution_input_accepted_for_continuation":
        reasons.append(f"route_specific_artifact_execution_continuation_record_not_accepted:{route_type}")
    if record.get("can_continue_to_route_specific_artifact_execution") is not True:
        reasons.append(f"route_specific_artifact_execution_continuation_record_cannot_continue:{route_type}")
    if record.get("command_path") != ARTIFACT_EXECUTION_COMMAND_PATH:
        reasons.append(f"route_specific_artifact_execution_continuation_command_path_mismatch:{route_type}")
    if record.get("next_report_path") != ARTIFACT_EXECUTION_REPORT_PATH:
        reasons.append(f"route_specific_artifact_execution_continuation_report_path_mismatch:{route_type}")
    if record.get("next_review_path") != ARTIFACT_EXECUTION_REVIEW_PATH:
        reasons.append(f"route_specific_artifact_execution_continuation_review_path_mismatch:{route_type}")
    for field in [
        "artifact_executor_report_path",
        "artifact_executor_review_path",
        "delegated_report_path",
        "delegated_review_path",
        "route_specific_command",
    ]:
        if not record.get(field):
            reasons.append(f"route_specific_artifact_execution_continuation_{field}_missing:{route_type}")
    return dedupe(reasons)
...
def build_manual_continuation_record_blocking_reasons(
    ...
    if record.get("review_status") != "product_review_packet_input_accepted_for_continuation":
...
def build_request_blocking_reasons(
    ...
    if mode not in VALID_MODES:
        return ["downstream_execute_result_continuation_result_review_continuation_mode_invalid"]
    if mode == "dry-run":
        return []
    reasons = []
    ...
    if not note.strip():
        reasons.append("downstream_execute_result_continuation_result_review_continuation_note_required")
    return reasons
...
def build_status(
    ...
    if source_reasons:
        return "blocked_by_manifested_routed_downstream_execute_result_continuation_result_review_continuation_gate_entry"
    if boundary_reasons:
        return "blocked_by_manifested_routed_downstream_execute_result_continuation_result_review_continuation_gate_entry_boundary"
    if contract_reasons:
        return "blocked_by_manifested_routed_downstream_execute_result_continuation_result_review_continuation_gate_entry_contract"
    if unavailable_reasons:
        return "blocked_by_downstream_execute_result_continuation_result_review_continuation_command_unavailable"
    if "downstream_execute_result_continuation_result_review_continuation_mode_invalid" in request_reasons:
        return "blocked_by_downstream_execute_result_continuation_result_review_continuation_execute_mode"
    if mode == "dry-run" and continuation_kind == "route_specific_artifact_execution_continuation":
    ...
    if "confirm_downstream_execute_result_continuation_required" in request_reasons:
        return "blocked_by_missing_downstream_execute_result_continuation_result_review_continuation_execute_confirmation"
    if request_reasons:
        return "blocked_by_downstream_execute_result_continuation_result_review_continuation_execute_metadata"
    if continuation_kind == "route_specific_artifact_execution_continuation":
...
def build_continuation_execute_command(project_root: Path, record: dict[str, Any]) -> list[str]:
    return [
        "python3",
        record.get("command_path", ARTIFACT_EXECUTION_COMMAND_PATH),
        "--project-root",
        str(project_root),
        "--route-specific-artifact-executor-entry-result-review",
        "derived_from_p7_bp_downstream_execute_result_continuation_result_review_continuation_gate_entry",
        "--mode",
        "dry-run",
        "--output-execution",
        record.get("next_report_path", ARTIFACT_EXECUTION_REPORT_PATH),
        "--output-review",
        record.get("next_review_path", ARTIFACT_EXECUTION_REVIEW_PATH),
    ]


def build_route_specific_artifact_execution_source(
    gate_entry: dict[str, Any],
    record: dict[str, Any],
) -> dict[str, Any]:
    ...
    return {
        ...
        "generated_at": utc_now(),
        "topic": gate_entry.get("topic", ""),
        "source_status": "derived_from_manifested_routed_downstream_execute_result_continuation_result_review_continuation_gate_entry",
        ...
        "route_specific_artifact_execution_records": [
            {
                "record_id": record.get("source_record_id", f"artifact_executor_dry_run::{route_type}"),
...
def build_product_review_packet_continuation_record(
    record: dict[str, Any],
    reviewer: str,
    note: str,
) -> dict[str, Any]:
...
def build_source_gate_entry_summary(gate_entry: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "downstream_execute_result_continuation_result_review_gate_entry_recorded": (
            gate_entry.get("downstream_execute_result_continuation_result_review_gate_entry_recorded") is True
        ),
        "can_request_downstream_execute_result_continuation_result_review_continuation": (
            gate_entry.get("can_request_downstream_execute_result_continuation_result_review_continuation") is True
        ),
...
def build_next_action(status: str, blocking_reasons: list[str], continuation_kind: str) -> dict[str, Any]:
    if status == EXPORT_DRY_RUN_STATUS:
        return {
            ...
            "label": "Confirm route-specific artifact execution continuation",
            "description": "Dry-run is ready; rerun with confirmation, reviewer, and note to enter route-specific artifact execution dry-run.",
        }
    if status == MANUAL_DRY_RUN_STATUS:
        return {
            "id": "rerun_with_confirm_product_review_packet_continuation",
            "label": "Confirm product-review packet continuation",
            "description": "Dry-run is ready; rerun with confirmation, reviewer, and note to record product-review packet continuation.",
        }
    if status == EXPORT_READY_TO_ENTER_STATUS:
        return {
            "id": "enter_route_specific_artifact_execution",
            "label": "Enter route-specific artifact execution",
            "description": "The route-specific artifact execution dry-run command is ready to run.",
        }
    if status == EXPORT_ENTERED_STATUS:
        return {
            "id": "review_route_specific_artifact_execution",
            "label": "Review route-specific artifact execution",
            "description": "P7-BP entered route-specific artifact execution dry-run.",
        }
    if status == MANUAL_RECORDED_STATUS:
        return {
            "id": "prepare_product_review_packet",
            "label": "Continue product-review packet",
            "description": "P7-BP recorded product-review packet continuation for the manual terminal branch.",
        }
    if status == (
        "blocked_by_missing_downstream_execute_result_continuation_result_review_continuation_execute_confirmation"
    ):
        return {
    ...
    if status == "blocked_by_downstream_execute_result_continuation_result_review_continuation_execute_metadata":
        return {
    ...
    if status == "blocked_by_manifested_routed_downstream_execute_result_continuation_result_review_continuation_gate_entry_contract":
        return {
    ...
    if status == "blocked_by_manifested_routed_downstream_execute_result_continuation_result_review_continuation_gate_entry_boundary":
        return {
    ...
    if status == (
        "blocked_by_downstream_execute_result_continuation_result_review_route_specific_artifact_execution_failure"
    ):
        return {
            "id": "repair_route_specific_artifact_execution_inputs",
            "label": "Repair route-specific artifact execution inputs",
            "description": "The route-specific artifact execution dry-run did not reach its ready state.",
            "blocking_reasons": blocking_reasons,
        }
    if status == "blocked_by_downstream_execute_result_continuation_result_review_continuation_command_unavailable":
        return {
    ...
    return {
        "id": "resolve_p7_bo_continuation_gate_entry_blockers",
        "label": "Resolve P7-BO blockers",
        "description": f"P7-BO must be ready before P7-BP can execute {continuation_kind or 'the continuation'}.",
        "blocking_reasons": blocking_reasons,
    }


def write_auto_mode_formal_package_next_gate_manifested_routed_downstream_execute_result_continuation_result_review_continuation_gate_entry_execute_gate_outputs(
    project_root: Path,
    report: dict[str, Any],
...
def render_review(report: dict[str, Any]) -> str:
    lines = [
        ...
        "- 可确认执行 downstream execute result continuation result review continuation："
        f"{str(report['can_execute_downstream_execute_result_continuation_result_review_continuation_with_confirmation']).lower()}",
        ...
        "- 已进入 route-specific artifact execution："
        f"{str(report['route_specific_artifact_execution_entered']).lower()}",
        "- product-review packet continuation 已记录："
        f"{str(report['product_review_packet_continuation_recorded']).lower()}",
        f"- product-review continuation records：{len(report['product_review_packet_continuation_records'])}",
    ...
    if report["continuation_execute_command"]:
        ...
        lines.append("```")
    if report["product_review_packet_continuation_records"]:
        lines.extend(["", "## Product Review Packet Continuation"])
        for record in report["product_review_packet_continuation_records"]:
            lines.append(f"- `{record['record_id']}`: {record['continuation_status']}")
