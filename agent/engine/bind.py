...
def format_estimate_facts(
    ...
    *,
    ...
    if ols_lock_active(state):
        # OLS hard lock: the projected formula must not carry TWFE wording.
    ...
    return "\n".join(
        [
            ...
            f"估计器：{display_estimate_engine_label(estimate.get('method'), estimate.get('estimator')) or shown_estimator}",
