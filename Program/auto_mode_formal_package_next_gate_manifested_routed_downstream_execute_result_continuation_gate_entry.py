...
from Program.workbench.auto_mode_formal_package_next_gate_manifested_routed_downstream_execute_result_continuation_gate_entry import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record a downstream execute result continuation gate entry.")
    parser.add_argument("--project-root", default=".")
    parser.add_argument(
        "--manifested-routed-downstream-execute-result-review",
    ...
    result_review_path = Path(args.manifested_routed_downstream_execute_result_review)
    ...
    report = build_auto_mode_formal_package_next_gate_manifested_routed_downstream_execute_result_continuation_gate_entry(
        result_review,
        source_paths={
            "manifested_routed_downstream_execute_result_review": str(result_review_path),
        },
    )
    ...
    print(
        "[econ-workbench] "
        f"auto_mode_formal_package_next_gate_manifested_routed_downstream_execute_result_continuation_gate_entry="
        f"{report_path.relative_to(project_root)}"
    )
    print(
        "[econ-workbench] "
        f"auto_mode_formal_package_next_gate_manifested_routed_downstream_execute_result_continuation_gate_entry_md="
