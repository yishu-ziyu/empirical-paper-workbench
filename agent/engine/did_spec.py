"""DID-BE-spec: when allow_did, force treated×period (or equivalent).

If the 2×2 main term is missing → hard block (no estimate / no
write-as-estimated). Does not set allow_did, does not emit TWFE as a
DiD substitute, and does not rewrite the OLS lock.

See docs/did-narrow-exception-contract.md §2.
"""
...
def allow_did_active(state: Mapping[str, Any] | None) -> bool:
    """Fail closed. Missing / null is false. Does not re-run the title gate."""
    return isinstance(state, dict) and state.get("allow_did") is True
...
def force_did_main_term(
    ...
    *,
    ...
    outcome = _first_str(
        base, "outcome", "outcome_col", "dv"
    ) or _first_str(rd, "outcome", "outcome_col", "dv")
    if not outcome:
        return None

    names = _candidate_names(base, rd, columns)
...
def can_form_did_main_term(
    ...
    """True when payload + session columns can name the 2×2 term."""
    spec = None
    ...
    if has_did_main_term(spec, rd, columns):
        return True
    return force_did_main_term(spec, columns=columns, direction=rd) is not None


def apply_did_spec(state: Mapping[str, Any] | None) -> dict[str, Any]:
    """When allow_did, force the 2×2 term onto main_specification."""
    if not allow_did_active(state):
        return {}
    ...
    columns = columns_from_state(state)
    forced = force_did_main_term(spec, columns=columns, direction=rd)
    if forced is None:
        return {}
...
def did_spec_block_reason(state: Mapping[str, Any] | None) -> str | None:
    """Hard-block code when allow_did and the confirmed spec lacks the term."""
    if not allow_did_active(state):
        return None
    assert isinstance(state, dict)
    spec = state.get("main_specification")
