...
from workbench.manuscript_section_scaffold import (
    build_manuscript_section_scaffold_report,
    write_manuscript_section_scaffold_report,
    write_manuscript_section_scaffold_review,
)
from workbench.paper_revision_round import snapshot_formal_state


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create draft-layer manuscript section scaffolds from section work orders.")
    ...
    parser.add_argument(
        ...
        help="Output scaffold report path relative to project root.",
    )
    parser.add_argument(
        "--output-review",
        default="Reviews/manuscript_section_scaffold.md",
...
def main() -> int:
    ...
    report = build_manuscript_section_scaffold_report(
        project_root,
        revision_round,
        revision_round_path,
        formal_state_before=formal_state_before,
    )
    report_path = write_manuscript_section_scaffold_report(output_report_path, report)
    review_path = write_manuscript_section_scaffold_review(output_review_path, report)

    print(f"[econ-workbench] manuscript_section_scaffold_report={report_path.relative_to(project_root)}")
    print(f"[econ-workbench] manuscript_section_scaffold_review={review_path.relative_to(project_root)}")
    print(f"[econ-workbench] section_scaffolds={report.get('section_count', 0)}")
