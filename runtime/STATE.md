...
| `cursor/did-be-gate-8102` | 已重做 | 顶端 `agent/engine/did_spec.py:51 did_spec_applies`、`:166 apply_did_spec`、`:185 did_spec_block_reason`；`backend/services/allow_did.py:105 session_allow_did`。顶端 `runtime/tasks/20260915-fm-e-build-did-gate-recut.md` 记「Did not revive `cursor/did-be-gate-8102` @ `71be39f1`」；分支旧口径（`catalog_identity_allows` / `title_topic_allows` 用目录令牌放行）被 DECIDE-6 主动撤销，顶端 `docs/did-narrow-exception-contract.md` 15 行存根写明 superseded |
...
| `docs/fm-e-build-data-complete-1-g0` | 真缺口 | `git cat-file -e <top>:docs/data-completion-contract.md` → exit 128；而顶端合同把它当 sister contract 引用（悬空引用）：`docs/find-data-lit-contract.md:8`、`:502`，`docs/infer-design-contract.md:320`、`:478`，`docs/real-fetch-contract.md:583` |
...
| `feat/fm-e-build-did-narrow-1` | 已重做 | own 唯一产物 `docs/did-narrow-exception-contract.md`（219 行，blob `64c79d5a`）在顶端是 15 行存根（blob `05c0df26`），正文明写「Superseded by: `docs/infer-design-contract.md` §7」；替代实现 `agent/engine/did_spec.py` / `backend/services/allow_did.py` |
...
| `fix/fm-e-takeaway-g0-contract` | 真缺口 | `git grep -i -n takeaway <top>` 0 命中；`git cat-file -e <top>:docs/takeaway-math-research-notes.md` → exit 128。该笔记自述状态 BLOCKED/OPEN，记录 MATH-1 在真机 Word 里公式仍是转义 LaTeX —— 与第 17 条合读：公式门既没落顶端，也没被验证 |
| `fix/ols-lock-twfe-phrasing` | 真缺口 | 顶端无 `agent/engine/ols_lock.py` / `agent/tests/test_ols_lock.py`（`git cat-file -e` exit 128）；`agent/prompts/methods.py` 无 OLS 硬锁注入，`agent/nodes/generate_chapter.py` 无 TWFE 清洗（grep `sanitize\|TWFE\|固定效应` 0 命中）。顶端只留合同文本 `docs/infer-design-contract.md:423` 与标签函数 `agent/design/spec.py:42`。详见 C3-1 |
...
- GitHub 侧：`gh issue view 24 --repo yishu-ziyu/empirical-paper-workbench --json state,title` → `{"state":"OPEN","title":"OLS lock still lets 双向固定效应 phrasing through"}`。顶端有的只是合同文字（`docs/infer-design-contract.md` §7.4 与 §10 的 OLS lock 行），而且那份表格把它列为「外部并行写集，G0 不得触碰」——即顶端自己记录了这个缺口尚未实现。
...
- 悬空合同引用：顶端 `docs/find-data-lit-contract.md`、`docs/infer-design-contract.md`、`docs/real-fetch-contract.md` 都把 `docs/data-completion-contract.md` 列为 sister contract，但该文件在顶端不存在。这是文档层面的不一致，需要用户决定是补文件还是改引用。
