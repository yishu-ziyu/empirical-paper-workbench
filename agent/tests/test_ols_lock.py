...
def test_estimate_ols_labels_ols_not_feols(tmp_path):
    ...
    # 9/15 recut keeps engine truth in state (``agent/design/spec.py``
    # ``display_estimate_engine_label`` docstring + ``agent/tests/test_estimate.py``
    # ``test_ols_results_label_is_ols_not_feols``); the OLS hard lock is enforced
    # on the fitted spec (no ``| FE``) and on every user-facing label below.
