...
- Change 来源（用户 2026-09-16 决策）: 「这些都要」——14 条缺口全部补齐；`docs/contracts/data-completion-contract.md` 悬空引用按「补文件」处理（该合同在 `docs/fm-e-build-data-complete-1-g0` 上存在）；#24 的锁严格度沿用 `fix/ols-lock-twfe-phrasing` 已定的口径（模块 docstring 第 14 行：`There is intentionally no negation window. Any hit is a TWFE claim.`）。
- Session / run ID:
