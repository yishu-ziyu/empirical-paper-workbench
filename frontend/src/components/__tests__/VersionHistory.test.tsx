...
describe('VersionHistory 版本历史下拉', () => {
  test('渲染所有版本项', () => {
    renderWithI18n(<VersionHistory {...baseProps} />)
  ...
  test('当前版本高亮（currentVersionIndex）', () => {
    renderWithI18n(<VersionHistory {...baseProps} currentVersionIndex={1} />)
    const items = screen.getAllByTestId('version-item')
  ...
  test('点击版本项触发 onSelectVersion 回调', async () => {
    ...
    renderWithI18n(<VersionHistory {...baseProps} onSelectVersion={onSelectVersion} />)
    const items = screen.getAllByTestId('version-item')
  ...
  test('空版本列表显示提示', () => {
    renderWithI18n(<VersionHistory versions={[]} onSelectVersion={vi.fn()} />)
