...
写作动作不能偷偷 Promote canonical。Claim 批准绑定当时的 evidence revision；新 SpecificationRun / Challenge / Promote / Revert 让旧 Claim 变 stale，stale Claim 不能 grounded Paper。Card 措辞边界用同一套 deterministic policy，而不是 exact sentence match。Compare 的 why_moved / changed / unchanged 以 backend 为准。无 admissible spec 时是受控业务失败，不是 NameError/500。
...
- [x] I3 Claim 绑定 evidence revision — 程序: backend 测 — 预期: lab 有 `evidence_revision`；draft/approve 的 claim 有 `based_on_evidence_revision`；新 completed spec_run / challenge run / promote / revert 使 revision +1；旧 claim 文本保留且 `stale=true`。
...
- I3/I4 `test_new_spec_run_stales_approved_claim` + `test_merge_spec_run_does_not_clobber_newer_claim_or_canonical` + browser Scenario B: preview bumps revision 3→4, Claim v4 text preserved and `stale=true`, Paper badge `未 grounded`, refresh still stale; Review new evidence → v5 based_on=4 → approve → grounded returns.
