...
def _confirm_design(client, session_id: str, title: str = "最低工资对就业的影响") -> dict:
...
def test_attach_and_confirm_attach_are_not_404(client):
    ...
    confirm = client.post(f"/sessions/{sid}/confirm-attach")
    assert confirm.status_code != 404, confirm.text
    assert confirm.status_code == 409
    assert confirm.json()["detail"]["code"] == "no_candidate"
...
def test_confirm_attach_after_confirmed_design_preserves_design(
    client, tmp_path, monkeypatch
):
    catalog = tmp_path / "classic-5"
    _write_catalog(catalog, "ck1994_long")
    monkeypatch.setenv("ECONPAPER_CLASSIC5_ROOT", str(catalog))

    sid = _create_session(client)
    design = _confirm_design(client, sid)
    ...
    assert snap["design"]["status"] == "confirmed"
    assert snap["design"]["confirmed"] is True
    assert snap["design"]["catalog_entry_id"] is None
    assert snap["design"]["method"] == design["method"]
    state = facade.get_state(sid)
    assert state["design"]["catalog_entry_id"] is None
    assert state["attach_candidate"]["entry_id"] == "ck1994_long"
    assert state.get("allow_did") is not True
