...
from Program.workbench.auto_mode_formal_package_next_gate_verified_route_next_gate_router_entry import (  # noqa: E402
    DEFAULT_ENTRY_PATH,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Enter the verified route next-gate router after P7-AW accepts the completion ledger."
    )
    parser.add_argument("--project-root", default=".")
    parser.add_argument(
        "--verified-route-completion-ledger-entry-result-review",
    ...
    result_review_path = Path(args.verified_route_completion_ledger_entry_result_review)
    result_review = load_json_or_empty(project_root / result_review_path)
    report, _exit_code = run_auto_mode_formal_package_next_gate_verified_route_next_gate_router_entry(
        project_root,
        result_review,
        source_paths={
            "verified_route_completion_ledger_entry_result_review": str(result_review_path),
        },
        repo_root=REPO_ROOT,
    )
    ...
    print(
        "[econ-workbench] "
        "auto_mode_formal_package_next_gate_verified_route_next_gate_router_entry="
        f"{report_path.relative_to(project_root)}"
    )
    print(
        "[econ-workbench] "
        "auto_mode_formal_package_next_gate_verified_route_next_gate_router_entry_review="
    ...
    print(
        "[econ-workbench] "
        "can_enter_verified_route_next_gate_router="
        f"{str(report['can_enter_verified_route_next_gate_router']).lower()}"
    )
    print(
        "[econ-workbench] "
        "verified_route_next_gate_router_entry_command_executed="
    ...
    print(
        "[econ-workbench] "
        f"verified_route_next_gate_router_status={report['verified_route_next_gate_router_status']}"
    )
