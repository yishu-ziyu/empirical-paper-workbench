...
## 2026-05-25：MVP 使用一个全局开始执行确认按钮

Decision: `start_execution` gate 在 MVP 阶段只使用一个全局确认按钮，不默认给数据、方法、写作等每个阶段都加弹窗确认。

Reason: 用户要求保持产品简单。Agent Activity Panel 的职责是把派工、证据要求、风险、正式层边界和预期产物讲清楚，然后让用户一次性决定是否进入真实数据/方法执行。

Rejected: 每个小任务完成后都弹框等待人工点击。原因是这会把 Auto Mode 变成低效表单流，破坏本地高效研究工作流。

Rejected: 每个阶段默认都有审批弹窗。原因是阶段级暂停/驳回应该留给高风险、阻塞或需要人工判断的阶段，而不是 MVP 的默认路径。

Evidence: 用户在 2026-05-25 明确推荐“MVP 先用一个全局确认按钮；后续高风险阶段再加阶段级暂停/驳回控制”。
