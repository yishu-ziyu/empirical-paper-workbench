...
Given 用户打开工作流仪表盘是为了快速理解项目卡在哪一支
When 页面加载完成
Then 首屏必须出现 `工作流树状路线图`
And 页面必须用 SVG 或等价图形结构展示 P12 到 P18 的节点、连线、阻断分支和禁止路径
And 当前节点 `P17`、下一步 `P18`、禁止动作 `不运行模型` 必须在图上有可见状态
And 大段文字摘要必须退到树图之后或侧边，不能成为第一视觉主体

验证的业务规则：看板的核心价值是让用户一眼看到流程树和卡点，而不是把状态文案堆成报告页。
